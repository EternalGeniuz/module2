...
class Command(BaseCommand):
    def handle(self, *args, **options):
        current_date = now()
        user = User.objects.get(username="eternal")
        ...
        for day_index in range(30):
            ...
            for slot_index in range(randint(5, 10)):
                start_date = current_date + timedelta(hours=randint(0, 12))
        ...
        for time_slot in saved_time_slots:
            ...
            for tag_index in range(count_of_tags):
                time_slot_tags.append(TimeSlot.tags.through(
                    timeslot_id=time_slot.id,
                    timeslottag_id=tags[tag_index].id
                ))
